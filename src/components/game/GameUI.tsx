...
const ResourceDisplay = ({ type, amount }: { type: ResourceType, amount: number }) => {
  const prevAmount = useRef(amount);
  const [isBumping, setIsBumping] = useState(false);

  useEffect(() => {
    if (Math.floor(amount) > Math.floor(prevAmount.current)) {
      setIsBumping(true);
      ...
      <AnimatePresence>
        {showMenu && (
          <motion.div 
            ...
            className="absolute top-16 left-3 w-64 bg-black/90 backdrop-blur-xl p-4 rounded-2xl border border-white/10 text-white shadow-2xl pointer-events-auto z-50 flex flex-col gap-4"
          >
            {/* Actions Section */}
            <div className="flex flex-col gap-2">
              ...
              <motion.button 
                whileHover={{ x: 5 }}
                onClick={() => { setShowResearch((v) => !v); setShowMenu(false); }} 
                className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 transition-colors"
              >
              ...
              <motion.button 
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-xl border border-white/5 transition-colors ${canFestival ? 'bg-pink-600/20 hover:bg-pink-600/30 text-pink-100' : 'bg-white/5 opacity-50 cursor-not-allowed'}`}
              >
                <PartyPopper className="w-4 h-4 text-pink-300" /> Festival
              </motion.button>

              <motion.button 
                whileHover={canExpedition ? { x: 5 } : {}}
                onClick={sendExpedition} 
                disabled={!canExpedition} 
                className={`flex items-center gap-3 px-3 py-2 rounded-xl border border-white/5 transition-colors ${canExpedition ? 'bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-100' : 'bg-white/5 opacity-50 cursor-not-allowed'}`}
              >
            ...
            <div className="flex flex-col gap-2">
              ...
              {user ? (
                ...
                <motion.button 
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={handleLogin} 
                  className="flex items-center gap-3 px-3 py-2 rounded-xl bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 text-blue-100 transition-colors"
                >
                    ...
                    {inProgress && (
                      <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                        <motion.div 
                          ...
                          animate={{ width: `${Math.min(100, researchProgress * 100)}%` }} 
                        />
                      </div>
                    )}
                              ...
                              <div className="flex flex-col gap-1">
                                  ...
                                  <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                                      <motion.div 
                                        ...
                                        initial={{ width: 0 }}
                                        animate={{ width: `${settler.hunger || 0}%` }}
                                      />
                                  </div>
                              </div>
                              <div className="flex flex-col gap-1">
              ...
              {(objectives || []).map((obj) => {
                ...
                return (
                  <motion.div 
                    layout
                    key={obj.id} 
                    className={`p-3 rounded-xl border ${complete ? 'border-green-400/40 bg-green-900/20' : 'border-white/10 bg-white/5'} shadow-inner`}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                    ...
                    <div className="mt-2 h-2 bg-white/10 rounded-full overflow-hidden">
                      <motion.div 
                        className={`h-full ${complete ? 'bg-green-400' : 'bg-blue-400'}`} 
                        initial={{ width: 0 }}
        ...
        <AnimatePresence>
          {showBuildMenu && (
            <motion.div 
              ...
              transition={{ type: 'spring', damping: 20, stiffness: 200 }}
              className="pointer-events-auto mx-auto w-full max-w-5xl bg-black/80 backdrop-blur-md p-4 rounded-2xl border border-white/10 shadow-2xl mt-12 mb-16 z-40"
            >
